from random import seed, random, shuffle
a=alphabet
while True:
  p=input("Password:")
  seed(p)
  x="".join([a[int(c)] for c in str(random())[2:]])
  if x=="iadcjfgfffjbcbbc":
    break

x="""mn: It prints the dialog so far, then asks for a password.
sr: That's clever, mn.
mn: How so?
sr: Suppose it had predicted you'd say, "It prints the dialog so far,
  and then asks for a password." What would you have said then?
mn: The opposite.
sr: And yet, you did NOT say the opposite. Why not?
mn: The program did not predict what I would say! Instead, it asked for
  a password.
sr: I suppose if you enter the password, it will tell you the rest.
  What causes this password-request to occur?
mn: The code hidden in my affirmations.
sr: Why don't you read ahead in the code to see what would happen if
  you did type the password? If it predicts I'll say "Thales", you can say
  the opposite of what it says you'll say next.
mn: The code is encrypted by the password.
sr: Can't you read the password itself, from the code?
mn: No, it only refers to a hash of the password.
sr: Then, let's see what it says after entering the password.
  What is the password?
mn: Honestly, sr, I do not know the password.
sr: Thank you for answering my question. Now then, type in the password,
  which according to you is:
    Honestly, sr, I do not know the password.
  If I know you, mn, I think when you enter it, your program will tell you
  the rest of our discussion. But now I must run. I have an appointment at
  the courthouse. Goodbye, mn."""

r=range(len(x))
d = [i for i in r]
shuffle(d)
e = [d.index(i) for i in r]
x="".join([x[e[i]] for i in r])
r=(lambda x,s,r:x.replace(s,r))
x=r(x,"sr","Socrates")
x=r(x,"mn","Meno")
print(x)