def F(newline, quote):
  dialog = [
    ...
    "Socrates: What would you like to ask me about?",
  ...
  no_grammar = {
    '<NO>': ['<no>', '<no>, <Socrates>'],
    '<no>': ['no', 'I do<nt> <think> so', 'it <seems> not', 'it does<nt> <seems> so'],
    '<Socrates>': ['Socrates', 'my friend', 'my <dear> friend'],
    '<dear>': ['dear', 'kind', 'wise', 'faithful', 'noble'],
    '<nt>': ["n't", ' not'],
    '<think>': ['suppose', 'think', 'believe'],
  ...
  def expand_grammar(grammar, line):
    results = []
    for token in grammar.keys():
      if token in line:
        matched = True
        for option in grammar[token]:
          modified_line = line.replace(token, option)
          results.extend(expand_grammar(grammar, modified_line))
  ...
  source = f"x={quote*3}{x}{quote*3}{newline}newline='''{newline}'''{newline}exec(x){newline}F(newline, '{quote}')"
  breakpoint()
  print("...")